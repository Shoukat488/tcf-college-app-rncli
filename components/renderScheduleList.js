...
const RenderScheduleList = () => {
  ...
  useEffect(() => {
    return () => {};
  }, [table, route, section, year]);

  const rendetItem = ({ item }) => {
    const selectedSection = section.filter((item) => {
      if (item.selected === true) return item.title;
    })[0].title;

    const sec =
      year === "XI"
        ? table.headerXI.indexOf(selectedSection)
        : table.headerXII.indexOf(selectedSection);
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        data={table[year] && section.length ? table[year][days[route]] : []}
